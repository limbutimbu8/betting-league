...
app.post('/api/register', async (req, res) => {
  try {
    ...
    if (!username || !password) return res.status(400).json({ success: false, message: 'Name and Password are required' });
    
    // Check if account with same email, phone or username exists
    const query = [ { username } ];
    if (email) query.push({ email });
    if (phone) query.push({ phone });
    
    const existing = await User.findOne({ $or: query });
    if (existing) return res.status(400).json({ success: false, message: 'Account already exists with this name, email or phone' });

    const hashedPassword = await bcrypt.hash(password, 10);
    const user = await User.create({ username, email, phone, password: hashedPassword, role: 'user', balance: 0 });
...
const otpStore = {}; // Memory store: { 'admin@gmail.com': { otp: '123456', expiresAt: 123456789 } }

app.post('/api/admin/request-otp', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const email = process.env.ADMIN_RECOVERY_EMAIL;
    if (!email || !process.env.RESEND_API_KEY) {
      return res.status(500).json({ success: false, message: 'OTP Email settings not configured in server' });
    }
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: email, // Must be clubpredicted@gmail.com for free tier
        subject: 'Prediction Club - Admin Password Reset OTP',
        html: `<div style="font-family:sans-serif; text-align:center; padding:20px;">
                 <h2>Prediction Club Security</h2>
...
app.post('/api/admin/verify-otp', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { otp } = req.body;
    const email = process.env.ADMIN_RECOVERY_EMAIL;
    
    if (!otp) return res.status(400).json({ success: false, message: 'OTP required' });
    if (!email || !otpStore[email]) return res.status(400).json({ success: false, message: 'No active OTP request found' });
    ...
    // OTP verified successfully (we map this to a verified token so the client can change password)
    res.json({ success: true, message: 'OTP verified', otpToken: otp });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});

app.post('/api/admin/verify-login', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { password } = req.body;
    if (!password) return res.status(400).json({ success: false, message: 'Password required' });

    const isMatch = await bcrypt.compare(password, req.user.password);
    if (!isMatch) return res.status(403).json({ success: false, message: 'Invalid admin password' });
...
app.post('/api/admin/change-password-otp', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { otpToken, newPassword } = req.body;
    const email = process.env.ADMIN_RECOVERY_EMAIL;

    if (!otpToken || !newPassword) return res.status(400).json({ success: false, message: 'Fields required' });
    if (!email || !otpStore[email] || otpStore[email].otp !== otpToken) {
      return res.status(403).json({ success: false, message: 'Invalid or expired OTP session' });
    }
    ...
    res.json({ success: true, message: 'Admin password changed successfully via OTP' });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
