(async function() {
  ...
  // === SECONDARY AUTHENTICATION LOGIC ===
  window.verifyAdminLogin = async function() {
    ...
    try {
      ...
      if(data.success) {
        ...
        document.getElementById('mainAdminApp').style.display = 'block';
        initDashboard(); // Fire up all specific admin resources
      } else {
      ...
      alert('Network error verifying login');
      btn.innerText = 'Verify Access';
      btn.disabled = false;
    }
  };
  ...
  window.toggleForgotView = function(showForgot) {
    if(showForgot) {
      document.getElementById('adminAuthLoginView').style.display = 'none';
      document.getElementById('adminAuthForgotView').style.display = 'block';
    } else {
      document.getElementById('adminAuthForgotView').style.display = 'none';
      document.getElementById('adminAuthLoginView').style.display = 'block';
      
      // Cleanup any inputs if they cancel/go back
  ...
  window.changeAdminPasswordOtp = async function() {
    ...
    try {
      ...
      if (data.success) {
        ...
        toggleForgotView(false); // Return to login overlay
      }
    } catch(e) {
